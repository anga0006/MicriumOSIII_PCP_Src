...
OS_PRIO OS_System_Ceiling = OS_CFG_PRIO_MAX-1;          /* Defining System Ceiling*/
/*
************************************************************************************************************************
*                                                   CREATE A MUTEX
*
                   ...
                   OS_ERR     *p_err)
{
    ...
    /*Case : If curret task requesting for mutex already owns it */
    ...
    /*  Case1: Active list of mutex pointer is null, so this is first task requesting for mutex. Grant it 
    *   Case2: A task with priority higher than the System Ceiling requests for Mutex. Grant it.
    *   p_mutex is pointer to current mutex requested.
    *   OSTCBCurPtr points to the TCB of task requesting the mutex.
    */
    if ( (OSMutexActiveListPtr == (OS_MUTEX *)0) || ((OS_PRIO)OSTCBCurPtr->TaskEntryArg < OS_System_Ceiling) ) {
        ...
        OS_MutexActiveListAdd(p_mutex);                     /*Adding the mutex to the active list */
        OS_System_Ceiling = p_mutex->ResourceCeil;          /*Setting the system ceiling*/    
    ...
    /* Case: If the task requesting for mutex is already holding a mutex then grant the mutex. 
     *       The linked list holding the list of the mutex which are held is iterated to check 
     *       if the current task is already holding a mutex.
     * Author: Team OS 
     */ 
    
    OS_MUTEX *p_active_list_head = OSMutexActiveListPtr;
      while (p_active_list_head != (OS_MUTEX *)0) {
        if (p_active_list_head->OwnerTCBPtr != (OS_TCB *) 0){
          if(p_active_list_head->OwnerTCBPtr == OSTCBCurPtr){
            p_mutex->OwnerTCBPtr       =  OSTCBCurPtr;          /* Yes, caller may proceed                               */ 
            p_mutex->OwnerOriginalPrio =  p_active_list_head->OwnerOriginalPrio;
            p_mutex->OwnerNestingCtr   = (OS_NESTING_CTR)1; 
            if (p_ts != (CPU_TS *)0) {
              *p_ts                   = p_mutex->TS;
            }
            OS_MutexActiveListAdd(p_mutex);                      /*Add the mutex to the active mutex list                */
        
            OS_System_Ceiling = p_mutex->ResourceCeil;           /*Set system ceiling to current Resource Ceiling        */      
        ...
        p_active_list_head = p_active_list_head->MutxActiveNextPtr;
      }
    ...
    /* Case: A task which requests a mutex but has priority less than or equal to system ceiling
     *       is blocked and the priority of the task which causes the block is raised to priority
     *       of the blocked task and added to the head of the ready list.
     * Author:  Team OS
     */
    p_active_list_head = OSMutexActiveListPtr;
    while(p_active_list_head != (OS_MUTEX *) 0)
    {
      if(p_active_list_head->OwnerTCBPtr != (OS_TCB *) 0)
      {
        //TODO: change to the deadline and not the priority......
       // if((p_active_list_head->OwnerTCBPtr->Prio) > OS_System_Ceiling)
        if((OS_PRIO)(p_active_list_head->OwnerTCBPtr->TaskEntryArg) > OS_System_Ceiling)
        {
            p_tcb = p_active_list_head->OwnerTCBPtr;
            OS_RdyListRemove(p_tcb);                   /* Remove from ready list at Current Mutex priority             */
            p_active_list_head->OwnerTCBPtr->Prio = OSTCBCurPtr->Prio;
            OS_PrioInsert(p_tcb->Prio);
    ...
    //TODO : Call the fucntion to add the current task to the head of the ready list...
                   ...
                   OS_ERR    *p_err)
{
    ...
    if (p_pend_list->NbrEntries == (OS_OBJ_QTY)0) {         /* Any task waiting on mutex?                             */
        ...
        OS_MutexActiveListRemove(p_mutex);                              /*Remove the mutex from active list - Team OS */
        OS_System_Ceiling = OSMutexActiveListPtr->ResourceCeil ;        /*Reassign System Ceiling - Team OS           */
    ...
    /*Remove the mutex from the list and Compute System Ceiling - Team OS */
    OS_MutexActiveListRemove(p_mutex);
    OS_System_Ceiling = OSMutexActiveListPtr->ResourceCeil ;
...
/*
************************************************************************************************************************
*                                          ADD/REMOVE MUTEX TO/FROM Active Mutex List
*
* Author     : Team OS  
* Description: These functions are called by uC/OS-III to add or remove a mutex to/from the active mutex list.
*
* Arguments  : act_mutex     is a pointer to the mutex to add/remove
*
* Returns    : none
*
* Note(s)    : These functions are INTERNAL to uC/OS-III and your application should not call it.
...
void  OS_MutexActiveListAdd (OS_MUTEX  *act_mutex)
{
    act_mutex->MutxActivePrevPtr               = (OS_MUTEX *)0;
    if (OSMutexActiveListPtr == (OS_MUTEX *)0) {
        act_mutex->MutxActiveNextPtr           = (OS_MUTEX *)0;
    } else {
        act_mutex->MutxActiveNextPtr           =  OSMutexActiveListPtr;
        OSMutexActiveListPtr->MutxActivePrevPtr =  act_mutex;
    }
    OSMutexActiveListPtr                 =  act_mutex;
}

void  OS_MutexActiveListRemove (OS_MUTEX  *act_mutex)
{
    OS_MUTEX  *act_mutex_next;
    OS_MUTEX  *act_mutex_prev;


    act_mutex_prev = act_mutex->MutxActivePrevPtr;
    act_mutex_next = act_mutex->MutxActiveNextPtr;

    if (act_mutex_prev == (OS_MUTEX *)0) {
        OSMutexActiveListPtr = act_mutex_next;
        if (act_mutex_next != (OS_MUTEX *)0) {
            act_mutex_next->MutxActivePrevPtr = (OS_MUTEX *)0;
        }
        act_mutex->MutxActiveNextPtr = (OS_MUTEX *)0;

    } else if (act_mutex_next == (OS_MUTEX *)0) {
        act_mutex_prev->MutxActiveNextPtr = (OS_MUTEX *)0;
        act_mutex->MutxActivePrevPtr      = (OS_MUTEX *)0;

    } else {
        act_mutex_prev->MutxActiveNextPtr =  act_mutex_next;
        act_mutex_next->MutxActivePrevPtr =  act_mutex_prev;
        act_mutex->MutxActiveNextPtr      = (OS_MUTEX *)0;
        act_mutex->MutxActivePrevPtr      = (OS_MUTEX *)0;
    }
}
...
#endif
    OSMutexActiveListPtr = (OS_MUTEX *)0;                    /* Active mutex linked list head pointer - Team OS       */  
